...
public class EventsCalendar {
	...
	public static long pushAppointmentsToCalender(Activity curActivity, Ride r, int status, boolean needReminder, boolean needMailService, String username, String email) throws Exception{
		...
		// its 1
		eventValues.put("title", r.getName());
		eventValues.put("description", "Ride:" + r.getId() );
		eventValues.put("eventLocation", r.getSource() + " To " + r.getDest());
		long endDate = r.getStartTime() + 1000 * 60 * 60; // For next 1hr
		Calendar c  = Calendar.getInstance();
		c.setTime(new Date(r.getStartTime()));
	...
	public static void removeAppointmentsFromCalender(Activity curActivity, Ride r, int status, String username, String email) throws Exception{
		...
		if (cursors.moveToFirst()){
			while (cursors.moveToNext())
			{
				...
				if (desc.equals("Ride:" + r.getId()) && (stDate == r.getStartTime())){
